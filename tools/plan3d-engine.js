...
function planSVG(layout, opts) {
  ...
  (layout.zones || []).forEach(function (z) {
    ...
    var lbl;
    if (zw < 150 && zh > zw) {
      // Narrow strip: run the name down the strip, anchored at the far end so it
      // never lands on top of the rack tag sitting in the middle.
      var lx = X(z.x) + 14, ly = Y(z.y + z.d) - 10;
      lbl = '<text x="' + lx.toFixed(1) + '" y="' + ly.toFixed(1) + '" font-size="11" font-weight="700" fill="' + c +
            '" text-anchor="end" transform="rotate(90 ' + lx.toFixed(1) + ' ' + ly.toFixed(1) + ')">' + esc(z.name) + '</text>';
    } else {
      lbl = '<text x="' + (X(z.x) + 8).toFixed(1) + '" y="' + (Y(z.y) + 20).toFixed(1) +
            '" font-size="13" font-weight="700" fill="' + c + '">' + esc(z.name) + '</text>';
    }
    o.push('<g><rect x="' + X(z.x).toFixed(1) + '" y="' + Y(z.y).toFixed(1) + '" width="' + zw.toFixed(1) + '" height="' + zh.toFixed(1) +
      '" fill="' + c + '" fill-opacity="' + (dark ? .18 : .10) + '" stroke="' + c + '" stroke-width="1.5" stroke-dasharray="8 5"/>' + lbl + '</g>');
  });
